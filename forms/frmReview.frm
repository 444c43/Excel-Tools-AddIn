...
Private Sub EvaluatePeriods()
    ...
    End Select
End Sub
Private Sub SetPeriodAndWeeks(period As String, weeks As Double)
...
Private Sub EvaluateAcctType()
    Select Case NewCustomer.AcctType
        Case "1 Wk"
            NewCustomer.Formula = "=ROUNDUP(SUM((RC[-2]/" & NewCustomer.ReviewWeeks & ")/RC[-4]),0)"
        Case "2 Wk"
            NewCustomer.Formula = "=ROUNDUP(SUM(((RC[-2]/" & NewCustomer.ReviewWeeks & ")/RC[-4])*2),0)"
        Case "3 Wk"
            NewCustomer.Formula = "=ROUNDUP(SUM(((RC[-2]/" & NewCustomer.ReviewWeeks & ")/RC[-4])*3),0)"
        Case "5 Day"
            NewCustomer.Formula = "=ROUNDUP(SUM(((RC[-2]/" & NewCustomer.ReviewWeeks & ")/RC[-4])/5),0)"
    End Select
End Sub
Private Function DateChk() As Boolean
    ...
    End If
End Function
Private Function CalculateWeeks() As Integer
